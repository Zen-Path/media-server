...
def run_gallery_dl_json(url: str) -> dict:
    """Runs gallery-dl in simulation mode to get JSON metadata."""
    try:
        # -s = simulate, -j = json
        result = subprocess.run(
            ["gallery-dl", "-s", "-j", url], capture_output=True, text=True, check=False
        )
        if result.returncode != 0:
            return {}
        # gallery-dl outputs multiple JSON objects (one per line usually)
        # or a list. We need to handle this strictly.
        # For this specific logic, we assume valid JSON output.
        return json.loads(result.stdout)
    except Exception as e:
        logger.error(f"Gallery-dl JSON fetch failed: {e}")
        return {}
